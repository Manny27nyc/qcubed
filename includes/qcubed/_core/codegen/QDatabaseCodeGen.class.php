...
//                        _p("ASSN Table: $strTableName<br />", false);
                        ...
                        $objTable = new QTable($strTableName, $this->strClassPrefix, $this->strClassSuffix);
...
//				$objOppositeColumn = clone($this->objTableArray[strtolower($objManyToManyReference->AssociatedTable)]->PrimaryKeyColumnArray[0]);
		...
		protected function AnalyzeTypeTable(QTypeTable $objTypeTable) {
            ...
            if ($objFieldArray) foreach ($objFieldArray as $objField) { 
                $objColumn = $this->AnalyzeTableColumn($objField, $objTypeTable); 
                if ($objColumn) {
                    $objColumnArray[strtolower($objColumn->Name)] = $objColumn; 
                }
            }
            ...
            while ($objRow = $objResult->FetchAssoc()) {
				...
				if (sizeof($objRow) > 2) { // there are extra columns to process
					...
					foreach($objRow as $strColumnName => $strColumnValue) {
						if ($strFirstColumnName == $strColumnName) {
							continue;
						}
                        $strFieldName = QCodeGen::TypeNameFromColumnName($strColumnName);
                        $strExtraFields[$strFieldName] = $strFieldName;
                        $strExtraPropertyArray[$objRow[$strFirstColumnName]][$strFieldName] = $strColumnValue;
					}
                }
